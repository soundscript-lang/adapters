...
test('bun-plugin package manifest exposes the expected dependency contract', () => {
  assert.equal(pkg.name, '@soundscript/bun-plugin');
  assert.equal(pkg.version, '0.1.12');
  assert.equal(pkg.dependencies?.['@soundscript/adapter-core'], undefined);
  assert.equal(pkg.peerDependencies['@soundscript/soundscript'], '^0.1.12');
