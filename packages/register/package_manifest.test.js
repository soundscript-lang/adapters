...
test('register package works as a Node --import preload for transformed TypeScript entrypoints', async () => {
  const projectRoot = await mkdtemp(join(tmpdir(), 'soundscript-register-ts-preload-'));
  const packRoot = await mkdtemp(join(tmpdir(), 'soundscript-register-ts-pack-'));
  const soundscriptStubRoot = await mkdtemp(join(tmpdir(), 'soundscript-register-ts-peer-'));
  await mkdir(join(projectRoot, 'src'), { recursive: true });
  await mkdir(join(soundscriptStubRoot, 'project-transform'), { recursive: true });
  await writeFile(
    join(projectRoot, 'package.json'),
    `${JSON.stringify({ name: 'register-ts-preload-test', private: true, type: 'module' }, null, 2)}\n`,
    'utf8',
  );
  await writeFile(
  ...
  const run = spawnSync(
    'node',
    ['--import', '@soundscript/register', './src/main.ts'],
    {
      cwd: projectRoot,
      encoding: 'utf8',
      stdio: 'pipe',
